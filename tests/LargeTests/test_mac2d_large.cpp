...
int MAC2DAxisymTest_NonSurfaceTension() {
    ...
    const int maxiter = 50, niterskip = 1, num_bc_grid = 3;
    ...
    MSolver->SetBC_U_2D("axisym", "neumann", "dirichlet", "dirichlet");
    MSolver->SetBC_V_2D("axisym", "neumann", "dirichlet", "dirichlet");
    MSolver->SetBC_P_2D("axisym", "pressure", "neumann", "neumann");

    MSolver->SetBCConstantUE(1.0);
    MSolver->SetBCConstantUS(1.0);
    MSolver->SetBCConstantUN(1.0);
    MSolver->SetBCConstantVE(0.0);
    MSolver->SetBCConstantVS(0.0);
    MSolver->SetBCConstantVN(0.0);
    ...
    // MSolver->SetPoissonSolver(POISSONTYPE::BICGSTAB);
    MSolver->SetPoissonSolver(POISSONTYPE::CG);
    const int poissonMaxIter = 10000;
    ...
    for (int i = 0; i < nr + 2 * num_bc_grid; i++)
    for (int j = 0; j < nz + 2 * num_bc_grid; j++) {
        MSolver->m_u[idx3_2D(nz, i, j)] = 1.0;
...
int MAC2DAxisymTest_StationaryBubble() {
    ...
    const int maxiter = 500, niterskip = 25, num_bc_grid = 3;
    const double maxtime = 1.0;
    const bool writeVTK = false;
    // length of each cell
    const double dr = lenR / nr, dz = lenZ / nz;
    ...
    MSolver = std::make_unique<MACSolver2DAxisym>(rhoH, rhoL, muH, muL, gConstant, GAxis,
        L, U, sigma, nr, nz, baseR, baseZ, lenR, lenZ,
        cfl, maxtime, maxiter, niterskip, num_bc_grid, writeVTK);
    MSolver->SetBC_U_2D("axisym", "wall", "neumnann", "neumann");
    MSolver->SetBC_V_2D("axisym", "wall", "neumnann", "neumann");
    MSolver->SetBC_P_2D("axisym", "wall", "neumann", "outlet");
...
int MAC2DAxisymTest_SmallAirBubbleRising() {
    ...
    MSolver = std::make_unique<MACSolver2DAxisym>(rhoH, rhoL, muH, muL, gConstant, GAxis,
        L, U, sigma, nr, nz, baseR, baseZ, lenR, lenZ,
        cfl, maxtime, maxiter, niterskip, num_bc_grid, writeVTK);
    MSolver->SetBC_U_2D("axisym", "neumann", "neumann", "neumann");
    MSolver->SetBC_V_2D("axisym", "neumann", "neumann", "neumann");
